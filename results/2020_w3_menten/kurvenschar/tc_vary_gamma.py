...
arr = np.arange(-1,1.02,0.02)

d_list = [update_dict(d, val, "gamma") for val in arr]
sim_names = ["gama"+str(val) for val in arr]
...
cbar = g.fig.colorbar(sm, ax = g.axes, ticks = [-1, 0, 1.])
cbar.set_label(r"$\gamma$")

g.savefig("../figures/prolif_tc_gamma.pdf")
